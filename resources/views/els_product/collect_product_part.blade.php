...
<div class="header">
   <h1 class="page-header">
      <small>Collect Back Product Parts</small>
   </h1>
   <ol class="breadcrumb">
      <li><a href="{{url('/home')}}">Home</a></li>
      <li><a href="{{url('collect-parts-product-list')}}">Collect Back Product Parts</a></li>
      <li class="active">Collect Back Product Parts</li>
   </ol>
</div>
<div id="page-inner">
   <div class="row">
                     ...
                     <div class="panel panel-default">
                        <div class="panel-heading">
                           <div class="card-title">
                              <div class="title">Collect Back Product Parts</div>
                           </div>
                        </div>
                        <div class="panel-body">
                           {!! Form::open(['url' =>'collect_product_parts', 'class' => 'form-horizontal', 'role' => 'form','id' => 'barcode-allocation-form','onsubmit'=>'return false']) !!}
...
getBarcodeDetailsList=function()
{
	...
	if(barcode){
		$.ajax({
			type: "POST",
			url: "{{url(route('collect-product-parts-details'))}}",
...
checkBarcode=function(id){
	...
	if(barcode && part_id){
		$.ajax({
			type: "POST",
			url: "{{url(route('collect-barcode-details'))}}",
...
deleteRemove=function(e){
	...
	if(confirm("Are you sure you want to delete?")){
		...
		if(request_part_id){			
			$.ajax({
				type: "POST",
				url: "{{url(route('remove-collect-barcode'))}}",
...
addMore=function(id){
	...
	if(barcode){
		$.ajax({
			type: "POST",
			url: "{{url(route('add-more-collect'))}}",
...
getPartColour=function(id){
	...
	if(model_id && part_id){
		$.ajax({
			type: "POST",
			url: "{{url(route('get-collect-colours'))}}",
